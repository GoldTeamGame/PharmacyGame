...
public class PharmacistCounter
{
    ...
    public bool isPharmacist; // Tells customers if a pharmacist is working at the counter
    ...
    public PharmacistCounter(Position p)
    {
        ...
        path = new Node[3][];
        pharmacistZone[0] = p;
        Node[] newP1 = { new Node(new Position(pharmacistZone[0].x + 0.5f, pharmacistZone[0].y - 0.5f), 0, 6), new Node(new Position(pharmacistZone[0].x + 1, pharmacistZone[0].y - 0.5f), 0, 0)};
        path[0] = newP1;

        pharmacistZone[1] = new Position(p.x + 1, p.y - 0.5f);
        Node[] newP2 = { new Node(new Position(pharmacistZone[1].x - 0.5f, pharmacistZone[1].y - 0.5f), 0, 7), new Node(new Position(pharmacistZone[1].x - 1, pharmacistZone[1].y - 0.5f), 0, 1) };
        path[1] = newP2;
        ...
        lineStart = new Position(p.x + 1, p.y + 1.5f);
